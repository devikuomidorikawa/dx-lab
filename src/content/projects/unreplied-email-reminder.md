...
【毎朝自動実行（GAS のトリガー）】
  GAS が Gmail を検索
    → 直近3日間の受信メールのスレッドを取得
      → 各スレッドで自分が返信しているかチェック
        → 未返信のメールをスプレッドシートに書き出し

【スプレッドシート上で自動処理】
  =AI() 関数がメール内容を分析し「要返信 / 不要」を判定
    → 非勤務日を除外した経過時間を計算
      → 24時間以上 or 48時間以上の未返信をハイライト

【通知】
  GAS がスプレッドシートの「要返信」行を読み取り
    → Gmail or Google Chat でリマインド通知を送信
```
...
**判定の考え方:**
- 自分宛の質問・依頼・確認 → 「要返信」
- 通知メール（システム通知、カレンダー通知） → 「不要」
- CC で共有されただけのメール → 「不要」
- メーリングリスト経由の一斉メール → 「不要」
- 自動送信メール（no-reply アドレスなど） → 「不要」
